...
class photoLibWindow(QWidget):
    def __init__(self):
        ...
        self.media = mediaFileUtils(directory = QFileDialog.getExistingDirectory(self, 'Open file'))    
        self.initDB()
        self.initUI()
        return
...
#        for file in tqdm(self.files):
#            date, md5hash, mime, errors = self.getFileData(file)
#            self.insertDB(md5hash, file, mime, date, errors, self.dbCursor)
#        
#        self.dbConnect.close()
#        
#    def checkDB(self, password):
#        self.dbConnect = mariadb.connect(host="localhost", user="phillip", password=password, database='media')
#        self.dbConnect.autocommit = True
#        self.dbCursor = self.dbConnect.cursor()
#        self.missing = list()
#        for file in tqdm(self.files):
#            self.dbCursor.execute("SELECT md5hash, fileLocation FROM main WHERE fileLocation=?", (file,))
#            check = self.dbCursor.fetchall()
#            self.dbCursor.execute("SELECT md5hash, fileLocation FROM scratch WHERE fileLocation=?", (file,))
#            check.append(self.dbCursor.fetchall())
#            if not check:
...
#        
#    
#    def insertDB(self, md5, file, mime, date, errors, dbCursor):
#        try:
#            result = dbCursor.execute("INSERT INTO main (md5hash,fileLocation,mime,bestDate,errors) VALUES (?, ?, ?, ?, ?)", (md5, file, mime, date, errors))
#        except mariadb.Error as e:
#            error = f"{e}"
#            if error.find('Duplicate') != -1:
#                try:
#                    result = dbCursor.execute("INSERT INTO scratch (md5hash,fileLocation,mime,bestDate,errors) VALUES (?, ?, ?, ?, ?)", (md5, file, mime, date, errors))
#                    print('***************Duplicate******************')
#                except mariadb.Error as e:
#                    error = f"{e}"
#                    if error.find('Duplicate') != -1:
